...
type LinkType = "details" | "gallery" | "employees"; // Link türü belirtildi

export default function Fitness() {
    const [activeLink, setActiveLink] = useState<LinkType | null>(null); // activeLink'in türü belirtildi

    const handleLinkClick = (link: LinkType) => { // link parametresinin türü belirtildi
        setActiveLink(link);
    };

    useEffect(() => {
        // Sayfa yüklendiğinde "details" linkini otomatik olarak aktif hale getir
    ...
    return (
        <div>
            ...
            <div className='flex justify-center items-center gap-7 sm:gap-20 md:gap-40 lg:gap-56 py-5'>
                <div className={`border-b  hover:border-[#00A3FF] text-[20px] sm:text-[24px] lg:text-[30px] transition-all ${activeLink === 'details' ? 'border-[#00A3FF]' : ''}`}>
                    <Link to='details' onClick={() => handleLinkClick('details')}>Təfərrüatlar</Link>
                </div>
                <div className={`border-b  hover:border-[#00A3FF] text-[20px] sm:text-[24px] lg:text-[30px] transition-all ${activeLink === 'gallery' ? 'border-[#00A3FF]' : ''}`}>
                    <Link to='gallery' onClick={() => handleLinkClick('gallery')}>Gallerya</Link>
                </div>
                <div className={`border-b  hover:border-[#00A3FF] text-[20px] sm:text-[24px] lg:text-[30px] transition-all ${activeLink === 'employees' ? 'border-[#00A3FF]' : ''}`}>
